...
const EngineerCard = ({ engineer, onClick }: EngineerCardProps) => {
  const getAvailabilityColor = () => {
    if (engineer.currentProjects === 0) return 'bg-green-100 text-green-800 border-green-200';
    if (engineer.currentProjects === 1) return 'bg-yellow-100 text-yellow-800 border-yellow-200';
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-lg font-semibold text-foreground">{engineer.name}</h3>
          <p className="text-sm text-muted-foreground">{engineer.email}</p>
        </div>
        <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getAvailabilityColor()}`}>
          {getAvailabilityText()}
        </span>
      </div>
      ...
      <div className="space-y-3 mb-4">
        ...
        <div className="flex items-center text-sm text-muted-foreground">
          ...
          <span>{engineer.currentProjects}/{engineer.maxProjects} Projects</span>
        </div>
      </div>
